...
export default function LeaguePage() {
  ...
  const { hasNextPage, isFetchingNextPage, fetchNextPage } = usersData;
  ...
  if (leagueInfoQuery.isFetching || seasonInfoQuery.isLoading) return <div>로딩중</div>;
  if (leagueInfoQuery.isError || seasonInfoQuery.isError) return <div>에러 발생</div>;
  if (!leagueInfoQuery.data || !seasonInfoQuery.data) return <div>데이터 없음</div>;

  const users = usersData.data?.pages.flatMap((p) => p.contents.map(mapToUser)) ?? [];
  ...
  const showContent =
    !showWaitingTab &&
    !(seasonInfo.containsPopup && seasonInfo.lastSeasonPopupDto && !modalChecked);
      ...
      {showContent && (
        <div className="flex-grow py-20 flex lg:flex-row flex-col gap-5.5">
          ...
          <section className="flex flex-col gap-4 lg:w-1/2 w-full min-w-0 px-10">
            <div className="flex flex-col items-center gap-4">
              ...
              <div className="flex py-3 px-14 gap-6 rounded-[140px] bg-[#FFFFFF66]">
                ...
                <span className="font-bold text-[40px] text-white">{remainingTime}</span>
              </div>
            </div>
          ...
          >
            ...
            {!usersData.hasNextPage && <p className="text-gray-500">더 이상 유저가 없습니다.</p>}
          </section>
        </div>
      )}
    </div>
