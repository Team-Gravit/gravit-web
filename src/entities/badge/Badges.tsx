  ...
  return (
    <section className="flex flex-col py-5 px-4">
      <div className="flex items-end gap-1.5 text-[25px] font-semibold mb-6">
        ...
        <span className="text-[15px] text-[#494949]">{userBadges.earnedCount}개</span>
        ...
        <section key={category.categoryId}>
          <div className="flex flex-row items-center gap-2">
            ...
            {category.categoryDescription && (
              <p className="text-[#6D6D6D] text-[13.5px]">* {category.categoryDescription}</p>
            )}
          </div>

          <div className="grid grid-cols-5 gap-3 w-full bg-[#F2F2F2] rounded-xl p-3">
            {category.badgeResponses.map((badge) => (
              <div key={badge.badgeId} className="flex flex-col items-center text-center">
                {badge.earned ? <GoldBadge className="p-0.5" /> : <CopperBadge className="p-0.5" />}
                <span className="text-[19px] font-medium text-[#494949]">{badge.name}</span>
              </div>
            ))}
          </div>
