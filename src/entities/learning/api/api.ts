...
export const learningApi = {
  ...
  getChapterList: async (): Promise<Chapter[]> => {
    const response = await privateApiClient.get<ApiResponse<Chapter[]>>(`/learning/chapters`);
  ...
  getUnitList: async (chapterId: number): Promise<ChapterDetailResponseDTO> => {
    const response = await privateApiClient.get<ApiResponse<ChapterDetailResponseDTO>>(
  ...
  getProblems: async (lessonId: number): Promise<ProblemListResponseDTO> => {
    const response = await privateApiClient.get<ApiResponse<ProblemListResponseDTO>>(
      `/learning/${lessonId}`,
    );
