...
export default function WeeklyBarChart({
  ...
  lessonData: { day: string; count: number }[];
  maxCount: number;
}) {
  return (
    <ol className="w-full flex gap-2 md:gap-3 justify-between flex-1 mb-[42px] md:mb-0">
      {lessonData.map((data) => {
        ...
        return (
          <li key={data.day} className="flex-1 h-full flex flex-col justify-end">
            <LessonCompletionBar day={data.day} count={data.count} heightPercent={heightPercent} />
          </li>
        );
      })}
    </ol>
...
function LessonCompletionBar({ day, count, heightPercent }: BarProps) {
  ...
  return (
    <div className="flex flex-col gap-1 md:gap-2 h-40 md:h-full justify-end items-center">
      <span className={cn('text-caption1 text-main-1', count === 0 && 'text-text-4')}>
        {count}개
      </span>
      <div
