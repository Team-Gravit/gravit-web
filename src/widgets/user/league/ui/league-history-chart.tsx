...
function LeagueHistoryChart() {
	...
	// 실제 차트가 그려지는 범위
	const PLOT_WIDTH =
		chartConfig.width - chartConfig.padding.left - chartConfig.padding.right;
	const PLOT_HEIGHT =
	...
	const getY = (value: number) => {
		return (
			chartConfig.padding.top + ((value - minY) / (maxY - minY)) * PLOT_HEIGHT
		);
	};
	...
	const activeChartPoint = chartPoints.find(
		(point) => point.season === activePoint,
	);

	const activePointIndex = chartPoints.findIndex(
		(point) => point.season === activePoint,
	);
