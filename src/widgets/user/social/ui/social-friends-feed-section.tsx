import type {
	FriendActivityFeed,
	FriendActivityFeedList,
} from "@/entities/friends/model/types";
...
type FeedInfiniteResponse = InfiniteData<
	FriendActivityFeedList,
	number | undefined
>;
...
export default function SocialFriendsFeedSection() {
	...
	const friendFeeds =
		friendFeedData?.pages.flatMap((page) => page.contents ?? []) ?? [];
				...
				>
					{friendFeeds.length > 0 ? (
						<>
							...
							{hasNextPage && (
								<div ref={loadMoreRef} className="h-4 shrink-0" aria-hidden />
							)}
						</>
					) : (
						// TODO : Fallback UI 요청
						<div className="min-h-[150px] flex items-center justify-center">
							<p className="text-text-4 text-label1 md:text-heading2">
								최근 친구 활동이 없습니다
							</p>
						</div>
					)}
			...
			{feeds.map((feed, idx) => (
				<Fragment key={feed.feedId}>
					...
					{idx < feeds.length - 1 && (
						<div
							aria-hidden
							className="min-h-px w-full bg-divider-1 md:hidden"
						/>
					)}
				</Fragment>
			))}
