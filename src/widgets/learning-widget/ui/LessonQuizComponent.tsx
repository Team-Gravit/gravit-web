...
export default function LessonQuizComponent({ lessonId }: { lessonId: string }) {
  ...
  //학습 결과 제출 상태
  const isSubmittingResult = useQuizSessionState((state) => state.isSubmittingResult);
    ...
    <>
      ...
      {!isQuizCompleted && (
        <div className="w-full h-screen flex flex-col">
          <QuizHeader learningTitle={data.unitSummary.title} onHandleQuit={handleClickQuit} />
          ...
          <main className=" bg-gray-200 flex flex-col items-center h-full">
            <div className="flex flex-col gap-15 w-full h-full max-w-[1500px] 3xl:w-[80%] pt-15 px-10 lg:px-20">
              <ProblemStatement problem={currentProblem} totalProblemsCount={totalProblems} />
              <AnswerInteraction
