...
export default function AnswerInteraction({
  ...
  const saveSubmitResponse = useQuizSessionState((state) => state.saveSubmitResponse);
  ...
  const handleButtonClick = async () => {
    ...
    if (!isSubmitted) {
      ...
      // 주관식 답이 있는 경우
      if (answer) {
        saveUserAnswer(answer.answer, answer.isCorrect, problem.problemId, 'SUBJECTIVE');
  ...
  return (
    <section className="relative flex flex-col w-full gap-6 flex-grow justify-between">
      {!isSubmitted ? (
        ...
        <ReviewPhase problem={problem} userAnswer={userAnswers[currentProblemIndex]} />
      )}
