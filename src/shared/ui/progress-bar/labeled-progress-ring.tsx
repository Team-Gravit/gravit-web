...
export default function LabeledProgressRing({
  ...
  return (
    <div className={cn('flex items-center', labeledProgressRingSize[size].gapSize, className)}>
      <ProgressRing value={value} size={labeledProgressRingSize[size].ringSize}>
        {content}
      </ProgressRing>

      <span className={cn(`${labeledProgressRingSize[size].labelSize}`, labelClassName)}>
        {label}
      </span>
    </div>
  );
