...
export default function CalendarHeatmapCell({ level, date, count }: CalendarHeatmapCellProps) {
  if (!date) {
  ...
  return (
    <div
      className={cn('size-3 rounded-xs md:rounded-sm md:size-4', HEATMAP_COLOR_LEVELS[level])}
      data-date={date}
