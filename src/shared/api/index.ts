...
export const api = {
  private: {
    ...
    withdrawal: new UserDeletionAPIApi(privateApiConfiguration, undefined, privateApiClient),
    // CS 노트
    note: new CSNoteAPIApi(privateApiConfiguration, undefined, privateApiClient),
    // 리그 관련
    league: new LeagueAPIApi(privateApiConfiguration, undefined, privateApiClient),
    myLeagueProfile: new MyLeagueProfileAPIApi(
      privateApiConfiguration,
      undefined,
      privateApiClient,
    ),
    leagueRank: new LeagueRankAPIApi(privateApiConfiguration, undefined, privateApiClient),
    // 유저 관리
    user: new UserAPIApi(privateApiConfiguration, undefined, privateApiClient),
    // 친구 관련
    friend: new FriendAPIApi(privateApiConfiguration, undefined, privateApiClient),
    friendSearch: new FriendSearchAPIApi(privateApiConfiguration, undefined, privateApiClient),
    // 학습 관련 - 새로운 엔드포인트
    chapter: new ChapterAPIApi(privateApiConfiguration, undefined, privateApiClient),
    unit: new UnitAPIApi(privateApiConfiguration, undefined, privateApiClient),
    lesson: new LessonAPIApi(privateApiConfiguration, undefined, privateApiClient),
    problem: new ProblemAPIApi(privateApiConfiguration, undefined, privateApiClient),
    // 북마크
    bookmark: new BookmarkAPIApi(privateApiConfiguration, undefined, privateApiClient),
    // 오답노트
    ...
    // 신고
    report: new ReportAPIApi(privateApiConfiguration, undefined, privateApiClient),
    // 배지
    badge: new BadgeApi(privateApiConfiguration, undefined, privateApiClient),
    // 공지사항
    notice: new NoticeQueryAPIApi(privateApiConfiguration, undefined, privateApiClient),
    // 토큰 재발급 (Access Token)
    authToken: new AuthTokenAPIApi(privateApiConfiguration, undefined, privateApiClient),
  },
  public: {
    // OAuth
    oAuth: new OAuth20APIApi(publicApiConfiguration, undefined, publicApiClient),
    // 토큰 갱신 (Refresh)
    authToken: new AuthTokenAPIApi(publicApiConfiguration, undefined, publicApiClient),
    // 계정 복구
    userRestore: new UserAPIApi(publicApiConfiguration, undefined, publicApiClient),
  },
} as const;
