...
export default function ReportResultModal({ type }: { type: 'confirm' | 'error' }) {
  const { isResultOpen, closeResultModal } = useLessonModalStore();
  const title = type === 'confirm' ? '회원님의 신고가 접수되었어요.' : '에러가 발생했습니다.';
  const content =
    type === 'confirm'
      ? [
          '회원님의 소중한 의견을 모아',
  ...
  return (
    <Modal isOpen={isResultOpen} hasCloseButton={false} onClose={closeResultModal}>
      <div className="bg-white rounded-2xl w-[500px] lg:w-[600px] flex flex-col items-center pt-[85px] overflow-clip">
        ...
        <h3 className="text-black font-semibold text-2xl tablet-3xl mb-5 mt-[40px]">{title}</h3>
