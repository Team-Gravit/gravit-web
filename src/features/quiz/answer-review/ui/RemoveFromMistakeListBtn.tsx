...
export default function RemoveFromIncorrectListBtn({
	...
	const { mutate: removeIncorrectProblem, isPending } =
		useRemoveIncorrectProblem();

	return (
		<button
			type="button"
			className={cn(
				"text-gray-500 font-semibold text-2xl underline cursor-pointer hover:text-gray-400 disabled:opacity-50 disabled:cursor-not-allowed",
				isRemoved && "hidden",
			)}
			disabled={isPending}
			onClick={() => {
