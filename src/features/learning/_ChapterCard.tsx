  ...
  return (
    ...
    >
      <div className="flex flex-row justify-between lg:mb-3 z-10">
        <h3 className={cn('text-3xl xl:text-4xl font-mbc text-white', !isActive && 'opacity-60')}>
          {title}
        </h3>
        {isActive && (
          <Tooltip
          ...
          >
            <div className="relative z-50 flex items-center justify-center gap-2.5 w-[300px]">
              ...
              <p className="text-white text-[16px] font-normal flex-wrap">{description}</p>
            </div>
          </Tooltip>
        )}
      </div>
      {isActive && <LessonProgressBar progressRate={progressRate} className="w-full" />}
      <img
