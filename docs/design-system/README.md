...
북마크·오답노트처럼 강조가 필요한 아이콘에는 `icon-accent`(`#ffb608`)를 쓴다. 이 색은 경고 상태가
아니므로 `semantic-warning`과 구분한다. 기존 Figma 이름 `color/3`을 그대로 옮긴 `--color-3`은
Tailwind의 `ring-3` 두께 유틸리티와 충돌하므로 사용하지 않는다.

### 3-6. 카드 외곽선

흰색 카드의 외곽선에는 `card-border`(`rgb(251 241 255 / 60%)`)를 쓴다. 배경과 경계를 은은하게
구분하기 위한 색이므로 일반 구분선인 `divider-*`와 구분한다.
