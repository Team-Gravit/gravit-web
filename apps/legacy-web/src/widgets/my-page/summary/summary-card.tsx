    ...
    <Card className="w-full py-4 md:py-[33px] px-0 md:px-4">
      <div className="flex flex-col gap-2 items-center md:hidden">
        ...
        <div className="flex flex-col items-center">
          <h3 className="text-title3 text-main-1">상위 {learningSummary.topPercent}%</h3>
      ...
      <div className="w-full flex flex-row items-center gap-0 px-4 py-3 md:px-0 md:py-0">
        ...
        <SummaryCardItem
          ...
          label="완료 레슨"
        />
        <SummaryCardItem value={learningSummary.totalLearningHours} unit="h" label="총 학습시간" />
        <SummaryCardItem value={learningSummary.averageAccuracy} unit="%" label="평균 정답률" />
      </div>
    </Card>
  );
}
...
function SummaryCardItem({ value, subValue, label, unit = '개', className }: SummaryCardItemProps) {
  return (
    ...
    >
      ...
      <div className="text-caption1 md:text-body1-normal text-text-4">{label}</div>
    </div>
  );
}
