...
function HeroContent({
  ...
  return (
    <div className={cn('mt-auto z-10 w-full', heroContentSpacing[spacing], className)}>
      {children}
    </div>
  );
}

function HeroHeader({ children, className }: { children: React.ReactNode; className?: string }) {
  return <div className={cn('absolute top-0 left-0 p-5 z-10 w-full', className)}>{children}</div>;
}
