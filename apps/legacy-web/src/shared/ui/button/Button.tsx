...
export const Button = forwardRef<HTMLButtonElement, ButtonProps>(
  (
    { children, loading = false, className, disabled, loadingText = '로딩 중...', ...props },
    ref,
  ) => {
    let variantClassName: string;

    if (props.display === 'block') {
      const { variant, size, ...rest } = props as ButtonHTMLAttributes<HTMLButtonElement> &
        BlockButtonProps;
      variantClassName = blockButtonVariants({ variant, size, className });
      // rest는 아래 spread에서 사용
      return (
        <button ref={ref} disabled={loading || disabled} className={variantClassName} {...rest}>
