...
export const useRemoveIncorrectProblem = ({ unitId }: UseRemoveIncorrectProblemParams) => {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: async (problemId: number) => {
      const response = await api.private.wrongAnsweredNote.deleteWrongAnsweredProblem({
        problemId,
      });
