    ...
    <div className="w-full flex flex-col items-center gap-8">
      <div
        className={cn(
          ...
          userAnswer.isCorrect ? 'border-[#00A80B] text-[#00A80B]' : 'border-error text-error',
        )}
      >
      ...
      {userAnswer.isCorrect ? (
        <div className="w-full flex items-center justify-between">
          <small className="self-start text-[#00A80B] font-semibold text-2xl">정답입니다!</small>
          {mode === 'INCORRECT' && <RemoveFromIncorrectListBtn problemId={userAnswer.problemId} />}
        </div>
      ) : null}
    </div>
  );
