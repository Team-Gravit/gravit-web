...
export default function SeasonEndModal({ popup, onConfirm }: SeasonEndModalProps) {
  const profileBgColor = PROFILE_COLORS[popup.profileImgNumber as keyof typeof PROFILE_COLORS];
  ...
  return (
    <div className="bg-white rounded-[20px] w-full h-full p-14 flex flex-row items-center justify-center gap-40">
      <div className="relative flex flex-col items-center justify-center">
        <p className="text-[#8100B3] text-[57px] font-bold mb-8 text-shadow-lg">{popup.rank}등</p>
      ...
      <div className="flex flex-col items-center justify-center gap-6">
        ...
        <p className="text-2xl text-[#6D6D6D] font-normal">다음 시즌도 지금처럼 진행해주세요!</p>
        <button
          type="button"
