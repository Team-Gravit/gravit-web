...
export default function useToggleBookmark() {
  ...
  return useMutation({
    mutationFn: async ({ problemId, currentIsBookmarked }: UseToggleBookmarkParams) => {
      if (currentIsBookmarked) {
        await api.private.bookmark.deleteBookmark({ problemId });
        return { action: 'delete' as const };
      } else {
        await api.private.bookmark.addBookmark({ problemId });
    ...
    onSuccess: (result) => {
      const message =
        result.action === 'save' ? '북마크에 추가되었어요.' : '북마크에서 제거되었어요.';
