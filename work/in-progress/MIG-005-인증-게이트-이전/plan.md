...
| **legacy 온보딩이 서버 변경으로 깨진다**            | dev는 이미, prod 배포 시 신규 가입자가 온보딩 불가 | Issue 4의 `isOnboarded` 계약으로 대체한다. **서버 변경이 prod에 먼저 가면** 그지점에 별도 처리가 필요하다 |
...
- [ ] legacy 온보딩 파손이 Issue 4로 대체됐는지 확인 (별도 `FIX-` 항목을 만들지 않기로 함)
