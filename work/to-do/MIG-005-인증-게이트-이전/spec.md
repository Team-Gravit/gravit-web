...
| 2026-09-09 | Issue 2(토큰 자동 재발급) 구현 및 검증        | 검사 4종 통과, AC-1~7 대조 완료. 기준선 C3~C7 보존. Issue 3~5는 미착수                            | #196      |
| 2026-09-09 | Issue 3(소셜 로그인·로그아웃) 구현 및 검증    | 검사 4종 통과, AC-1~7 대조 완료. D8은 사용자 확정으로 변경, D7은 toast 부재로 미구현              | #201      |
| 2026-09-09 | Issue 4·5 남긴 채 `to-do`로 보류              | 로그인 화면(MIG-018)을 먼저 진행한다. Issue 4의 게이트가 미인증자를 `/`로 보내는데 `/`가 아직 스캐폴드라 게이트를 넣어도 도달 화면이 placeholder다. `work-management.md` §3의 in-progress 단일 유지를 위해 폴더를 되돌린다. **재개 조건: MIG-018 완료 후 Issue 4부터 이어서 착수** | MIG-018 · #203 |
