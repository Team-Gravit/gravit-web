...
> `feature-planner` 3단계 산출물. 근거는 `spec.md`의 확정된 규칙(R1~R13)·시안(D1~D19)·ADR-1~4다.
...
When `제출하기`를 클릭
Then `제출하고 있어요`가 1개 있고, 풀이 화면은 덮개 뒤에 남되 `inert`로 조작이 막힌다
(제출 중은 풀이 라우트가 갖는다 — ADR-2)

☑ **AC-4-1** (범위: 통합)
Given 같은 상태에서 `POST`가 500으로 실패
When 덮개가 걷힘
Then `제출에 실패했어요. 다시 시도해 주세요.`가 토스트로 뜨고 `inert`가 해제된다

> **개정 (2026-09-24)** — 원안은 「`로딩중...` 1개, 발문은 화면에 없다」였다. 그대로 구현하면
> **랜딩 로딩(`LoadingScreen`)을 재사용**하게 되는데, 같은 마스코트가 한 레슨에 두 번 나와
> 처음으로 되돌아간 것처럼 보인다. 랜딩 로딩에는 최소 2.5초 붙잡기(R12)가 붙어 있어 성격도
> 반대다 — 제출은 빠를수록 좋다.
>
> 더 큰 이유는 **실패 경로**다. 화면을 통째로 바꿨다가 되돌리면 제출이 안 된 것인지 읽기
> 어렵다. 덮개는 걷히기만 하므로 푼 자리가 그대로 남는다.
>
> 덮개는 시각만 가리므로 `inert`를 함께 건다. 그것 없이는 탭 이동과 스크린 리더가 뒤쪽 버튼에
> 그대로 닿는다.
>
> 실패를 알리는 수단이 원안에 없어 AC-4-1로 토스트를 추가했다.
...
When `제출하기`를 클릭
Then 현재 문제의 발문이 화면에 그대로 있고, 진행률이 `3/3`으로 유지된다 (풀이 화면에 남고 답을 잃지 않는다 — ADR-2)

> **개정 (2026-09-23)** — 원안은 `data-status="completed"` 항목이 **3개**라고 적었으나,
> `toProblemProgressStatuses`는 **현재 문제를 제출 여부와 무관하게 `current`로** 표시한다
> (그 함수의 주석에 명시된 의도다). 마지막 문제가 현재 문제이므로 `completed`는 2개가 맞다.
> AC의 의도는 「답을 잃지 않았다」이므로 그것을 곧바로 세는 진행률로 바꿨다.
>
> 재시도 수단도 뺐다. 실패 시 같은 `제출하기` 버튼을 다시 누르면 되므로 별도 재시도 UI가 없다.
...
☑ **AC-9** (범위: 통합) — **추가 (2026-09-25)**
Given 문제 2개 중 1번만 제출하고 마지막 문제로 이동한 상태
When `제출하기`를 클릭
Then `/learning/lessons/7/result/{id}`로 이동하고 제출 본문의 `accuracy`가 `50`이다
(마지막 문제를 풀지 않아도 끝낼 수 있다 — R14 · 확인 필요 7의 잠정 결정 · R6)

☑ **AC-9-1** (범위: 통합) — **추가 (2026-09-25)**
Given 마지막 문제가 주관식이고 입력칸에 답을 쓴 상태
When `다음 문제`를 클릭
Then 채점 결과가 보이고 결과 라우트로 이동하지 않는다. 한 번 더 눌러야 제출된다 (R2)

> **AC-9 신설과 버튼 레이블 정정 (2026-09-25)** — 구현 중 **마지막 문제에서 답을 비워 두면
> 버튼이 아무 일도 하지 않는** 상태였다. 레이블은 `다음 문제`인데 갈 곳이 없고, 제출 조건이
> 「마지막 문제 + 답 제출됨」이라 제출도 되지 않았다. 확인 필요 7이 이미 「미완료가 남아도
> 확인 없이 그대로 제출한다」로 잠정 결정돼 있어 **제출 조건에서 답 제출 여부를 뺐다.**
>
> 같은 검토에서 AC-4·5·6이 적고 있던 `학습 완료`를 실제 레이블인 **`제출하기`**로 고쳤다
> (`QUIZ_SUBMIT_LABEL`). 화면에 없는 문구를 Then에 적어 두면 검증이 통과해도 근거가 되지 않는다.
>
> 이 두 AC는 REF-041(주관식 초안을 세션으로 이전)과 같은 커밋에서 구현됐다. 초안이 입력
> 컴포넌트의 지역 상태에 있는 동안에는 하단 버튼이 「제출할 답이 있는가」를 알 수 없었다.
...
> ⚠️ **시안과 어긋난다 (2026-09-23).** 시안은 같은 `789XP` 옆에 **`LV13까지`**로 그려져 있다.
> 「LV13까지」는 다음 레벨을 가리켜야 말이 되는데 시안의 13은 `currentLevel` 쪽 값이다.
> **시안의 더미값 오류로 보고 `nextLevel`로 구현했다.** 디자이너 확인이 필요하다.
...
에셋 대기(확인 필요 4)는 **2026-09-23에 해소**돼 표에서 뺐다. 아래는 **구현을 끝낸 뒤에도
답을 기다리는 항목**이다. 모두 잠정 결정으로 진행했고 교체 지점을 한 곳에 모아 두었다.

| 이슈 | 기다리는 것                         | 확인 필요 | 잠정 결정으로 구현한 것                         |
| ---- | ----------------------------------- | --------- | ----------------------------------------------- |
| 1·6  | 닫기·결과 버튼 목적지               | 14 · 5    | 유닛 상세 · `/main` · `/learning/units/$unitId` |
| 2    | `오답입니다!` 라벨 위치 · 시안 갱신 | 13        | 선지 헤더에 둔다 (R9·R11 우선)                  |
| 6    | `LV{n}까지`가 가리키는 레벨         | —         | `nextLevel`. 시안 더미값 오류로 판단            |
| 6    | `isLevelUp`·`leagueName` 사용 시안  | 3         | 표시하지 않는다 (D19)                           |

**구현은 막히지 않았다.** 답이 오면 위 네 지점만 고친다. 화면 상태는
`docs/implementation-status.md`에서 `△`(검증 미완)로 관리한다.
