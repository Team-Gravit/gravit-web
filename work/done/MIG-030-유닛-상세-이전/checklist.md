...
| A2·A3 제목 `Unit{NN}`                   | ✅ `displayOrder` 로 `Unit01` 표기 (확인 필요 1 확정)     |
| A4 제목 아래 설명                       | ✅                                                        |
| A5·A6 카드 3종과 문구                   | ✅ AC-2 · 문구 6종 그대로                                 |
| A7 카드 항상 활성                       | ✅ 이 작업에서는 분기 없음. 확인 필요 3 은 `FIX-036` 판정 |
...
| `GET /api/v1/units/{chapterId}` 호출 제거 | legacy는 유닛 순번과 챕터명 때문에 불렀다. 둘 다 레슨 응답에 있다(`chapterSummary` · `displayOrder`). **요청이 2건에서 1건으로 줄었다**                       |
| ~~제목에 유닛명 사용~~ 철회               | `develop`(#235)이 `displayOrder`를 추가해 시안대로 `Unit01`을 쓴다. 챕터 유닛 목록과 같은 규칙이라 목록·상세의 번호가 일치한다 (spec.md Changelog 2026-09-18) |
